...
int Game::getPointsByDifficulty(int difficulty) const {
...
std::string Game::getDifficultyLabel(int difficulty) const {
...
void Game::start() {
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (timeout) {
            ...
            if (compareIgnoreCase(userAnswer, questions[i].answer)) {
                correct = true;
            }
            else {
                for (const std::string& alt : questions[i].alternatives) {
                    if (compareIgnoreCase(userAnswer, alt)) {
                        correct = true;
                        break;
                    }
                }
